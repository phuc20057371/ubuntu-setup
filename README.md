...
2. **Giai đoạn 2**: Cài đặt các công cụ phát triển phần mềm cơ bản (**Java 17 OpenJDK**, **Maven**, **Node.js 22 LTS**, **PostgreSQL 17**, **Docker & Docker Compose**, **Visual Studio Code**, và **DBeaver CE**).

> **💡 Tự động hóa:** Bạn có thể chạy các file script `remove_snap.sh` và `install_tools.sh` đi kèm trong thư mục này để thực hiện tự động các bước dưới đây thay vì gõ lệnh thủ công.
...
### Bước 1.7: Cài đặt GNOME Tools & Hỗ trợ Flatpak cho GNOME Software
Cài đặt các công cụ tùy biến giao diện và hỗ trợ quản lý Flatpak:
```bash
...
### Bước 2.7: Cài đặt Visual Studio Code
1. Thêm khóa GPG và kho lưu trữ của Microsoft:
   ```bash
...
2. Cài đặt VS Code:
   ...
   ```
3. (Tùy chọn) Tắt Telemetry của VS Code bằng cách sửa file `~/.config/Code/User/settings.json` và thêm:
   ```json
...
### Bước 2.8: Cài đặt DBeaver CE
1. Thêm khóa GPG và kho lưu trữ của DBeaver:
   ```bash
